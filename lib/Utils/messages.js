...
export const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, 'interactiveButtons')) {
        const msg = message;
        const interactiveMessage = { nativeFlowMessage: { buttons: msg.interactiveButtons } };
        // Always set body from body or text
        if (msg.body || msg.text)
            interactiveMessage.body = { text: msg.body || msg.text };
        if (msg.title) {
            ...
            const mediaContent = hasMedia ? await prepareWAMessageMedia(message, options) : {};
        ...
        const msg = message;
        const hydratedTemplate = { hydratedButtons: msg.templateButtons };
        if (hasNonNullishProperty(message, 'text')) {
        ...
        const thumbResult = message.thumbnail
            ? await generateThumbnail(message.thumbnail, 'image', options)
            : null;
        m.listMessage = {
            title: message.title,
            buttonText: message.buttonText,
        ...
        const msg = message;
        const interactiveMessage = { shopStorefrontMessage: { surface: msg.shop.surface, id: msg.shop.id } };
        if (msg.body)
            interactiveMessage.body = { text: msg.body };
        if (msg.title) {
            interactiveMessage.header = { title: msg.title, subtitle: msg.subtitle ?? null, hasMediaAttachment: false };
        }
        else if (msg.caption || msg.image || msg.video || msg.document) {
            ...
            const mediaContent = hasMedia ? await prepareWAMessageMedia(message, options) : {};
            interactiveMessage.body = { text: msg.caption ?? '' };
            interactiveMessage.header = { title: null, subtitle: msg.subtitle ?? null, hasMediaAttachment: !!hasMedia, ...mediaContent };
        }
        if (msg.footer)
            interactiveMessage.footer = { text: msg.footer };
        m = { interactiveMessage };
        ...
        const interactiveMessage = {
            collectionMessage: { bizJid: msg.collection.bizJid, id: msg.collection.id, messageVersion: msg.collection.version },
        };
        if (msg.body)
            interactiveMessage.body = { text: msg.body };
        if (msg.title) {
    ...
    else if (hasNonNullishProperty(message, 'cards') || hasNonNullishProperty(message, 'carousel')) {
        const carousel = message.carousel ?? message;
        const slides = await Promise.all(carousel.cards.map(async (slide) => {
        ...
        const toDate = (value) => {
            ...
            if (typeof value === 'string' && value) {
                ...
                return Number.isFinite(numeric) ? new Date(numeric < 1000000000000 ? numeric * 1000 : numeric) : new Date(value);
            }
            return undefined;
        };
    ...
    else if (hasNonNullishProperty(message, 'payment') || (hasNonNullishProperty(message, 'amount') && hasNonNullishProperty(message, 'currency'))) {
        const msg = message;
